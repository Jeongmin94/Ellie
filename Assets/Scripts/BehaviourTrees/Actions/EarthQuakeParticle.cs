...
public class EarthQuakeParticle : ActionNode
{
    ...
    protected override State OnUpdate()
    {
        ...
        var startPos = context.transform.position + context.transform.forward * forwardOffset.Value;
        var startRotation = context.transform.rotation;

        // 각 파티클에 대한 각도 계산
        var totalAngle = angle.Value;
        var angleStep = totalAngle / (particleCount.Value - 1);

        for (var i = 0; i < particleCount.Value; i++)
        {
            // 현재 파티클의 각도 계산
            var currentAngle = -totalAngle / 2 + angleStep * i;
            var rotation = Quaternion.Euler(0, currentAngle, 0) * context.transform.rotation;
