...
namespace Boss1
{
    public class TerrapupaMapObjectController : BaseController
    {
        ...
        private void OnDestroyedMana(IBaseEventPayload payload)
        {
            ...
            var actor = manaPayload.Sender.GetComponent<TerrapupaBehaviourController>();
            if (actor == null)
            {
                actor = manaPayload.Sender.GetComponent<TerrapupaStone>().Owner.GetComponent<TerrapupaBehaviourController>();
        ...
        private void OnDropMagicStalactite(IBaseEventPayload payload)
        {
            ...
            if (boss != null)
            {
                var actor = boss.GetComponent<TerrapupaBehaviourController>();
