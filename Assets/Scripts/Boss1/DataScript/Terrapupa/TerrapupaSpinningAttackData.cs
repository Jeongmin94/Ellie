...
namespace Boss1.DataScript.Terrapupa
{
    ...
    public class TerrapupaSpinningAttackData : BehaviourTreeData
    {
        [Title("사운드 설정")] 
        [InfoBox("회전공격 공격 사운드")]
        public string sound1 = "TerrapupaAttackHit";

        [InfoBox("회전공격 공격시작 사운드")] 
        public string sound2 = "TerrapupaRollStart";

        [Title("이펙트 설정")] [InfoBox("회전 공격 시 이펙트")]
        public GameObject spinningAttackEffect1;

        [Title("회전 공격")] 
        [InfoBox("플레이어에게 타겟팅 회전 속도")]
        public float rotationSpeed = 2.0f;
        [InfoBox("회전 공격 중 이동 속도")] 
        public float movementSpeed = 1.0f;
        [InfoBox("회전 공격 공격 적중 거리")] 
        public float effectiveRadius = 7.0f;
